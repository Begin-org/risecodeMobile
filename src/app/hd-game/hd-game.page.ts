...
export class HdGamePage {
  ...
  carregar(){
    ...
    var loop = setInterval(function(){
      ...
      if( ( (xcp>=xBasket && xcp<=xBasket+widthBasket) || (xcp+widthCp>=xBasket && xcp+widthCp<=xBasket+widthBasket)) && ycp>=yBasket){
      ...
      if( ( (xWr>=xBasket && xWr<=xBasket+widthBasket) || (xWr+widthWr>=xBasket && xWr+widthWr<=xBasket+widthBasket)) && yWr>=yBasket){
        marginWr = -5;
        errors++;
      ...
      if( ( (xUsb>=xBasket && xUsb<=xBasket+widthBasket) || (xUsb+widthUsb>=xBasket && xUsb+widthUsb<=xBasket+widthBasket)) && yUsb>=yBasket){
        marginUsb = -5;
        errors++;
      ...
      if( ( (xHd>=xBasket && xHd<=xBasket+widthBasket) || (xHd+widthHd>=xBasket && xHd+widthHd<=xBasket+widthBasket)) && yHd>=yBasket){
        marginHd = -5;
        score++;
